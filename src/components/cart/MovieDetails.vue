    ...
    <div class="p-6 max-w-full mx-auto bg-gray-900 text-white">
      ...
      <div v-if="movieStore.movie" class="flex flex-col md:flex-row">
        <div class="flex-shrink-0 mb-6 md:mb-0 md:w-1/3">
          <img :src="movieStore.movie.posterUrl" alt="Movie Poster"
        ...
        <div class="flex-1 md:pl-6">
          ...
          <div class="max-h-80 overflow-y-auto">
            <div v-for="season in movieStore.movie.seasons" :key="season.seasonNumber" class="mb-6">
              <h2 class="text-xl md:text-2xl font-semibold mb-2 cursor-pointer flex items-center justify-between p-2 rounded-lg bg-gray-800 hover:bg-gray-700 transition duration-300"
                  @click="movieStore.toggleSeason(season.seasonNumber)">
                {{ seasonTitle }} {{ season.seasonNumber }}
                <svg xmlns="http://www.w3.org/2000/svg"
                     :class="{ 'rotate-180': movieStore.selectedSeason === season.seasonNumber }"
                     class="w-5 h-5 transform transition-transform duration-300" viewBox="0 0 24 24" fill="currentColor"
                     aria-hidden="true">
                  <path fill-rule="evenodd"
                        d="M14.293 6.293a1 1 0 011.414 1.414L11.414 12l4.293 4.293a1 1 0 01-1.414 1.414l-5-5a1 1 0 010-1.414l5-5a1 1 0 011.414 0z"
                        clip-rule="evenodd"/>
              ...
              <ul v-if="movieStore.selectedSeason === season.seasonNumber" class="list-disc list-inside pl-4">
                <li v-for="episode in season.episodes" :key="episode.id" @click="movieStore.selectEpisode(episode)"
                    :class="{ 'text-red-500': movieStore.currentEpisode === episode }"
      ...
      <div v-if="movieStore.currentEpisode" ref="episodePlayer" class="mt-6">
        ...
        <iframe v-if="movieStore.currentEpisode.videoUrl" :src="getIframeSrc(movieStore.currentEpisode.videoUrl)"
                class="w-full h-[50vh] md:h-[80vh]" frameborder="0" allowfullscreen></iframe>
      </div>
      <div v-if="movieStore.currentEpisode" class="mt-6 flex justify-center space-x-4">
        <button
            @click="movieStore.playPreviousEpisode(movieStore.movie.seasons.find(s => s.seasonNumber === movieStore.selectedSeason))"
            class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition duration-300">
          {{ buttonPreviousEpisode }}
        </button>
        <button
            @click="movieStore.playNextEpisode(movieStore.movie.seasons.find(s => s.seasonNumber === movieStore.selectedSeason))"
            class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition duration-300">
          {{ buttonNextEpisode }}
        </button>
      </div>
